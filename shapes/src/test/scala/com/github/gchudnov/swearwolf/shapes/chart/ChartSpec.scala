...
import com.github.gchudnov.swearwolf.shapes.chart.internal.ChartDrawer
  ...
  override def spec: ZSpec[Environment, Failure] =
    suite("Chart")(
      test("uStep") {
        ...
        val actual = input.map(it => ChartDrawer.uStep(cy)(it))
      ...
      test("yStep") {
        ...
        val actual = input.map(it => ChartDrawer.yStep(sy, cy)(y0, y1)(it))
      ...
      test("dot - compile one line") {
        ...
        val actual   = ChartDrawer.compile(sz, ceilY, maxY)(input, ChartStyle.Dot)
        val expected = Seq("     ⣠⣶⣿")
      ...
      test("step - compile one line up") {
        ...
        val actual   = ChartDrawer.compile(sz, ceilY, maxY)(input, ChartStyle.Step)
        val expected = Seq(" ▁▂▃▄▅▆▇██")
      ...
      test("quad - compile one line up") {
        ...
        val actual   = ChartDrawer.compile(sz, ceilY, maxY)(input, ChartStyle.Quad)
        val expected = Seq("▗▄▟██")
