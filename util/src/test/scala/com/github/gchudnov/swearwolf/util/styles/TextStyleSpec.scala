  ...
  override def spec: ZSpec[Environment, Failure] =
    suite("TextStyle")(
      ...
      test("compose style-seq and style-seq") {
        val xs       = Bold | Italic
        val ys       = Blink | Foreground(EitherColor.Black)
        val actual   = xs | ys
        val expected = TextStyleSeq(Seq(Bold, Italic, Blink, Foreground(EitherColor.Black)))
      ...
      test("contains in composite style on top-level") {
        val xs = TextStyleSeq(Seq(Bold, Italic, TextStyleSeq(Seq(Blink, Foreground(EitherColor.Black)))))
      ...
      test("contains in composite style on low-level") {
        ...
        val actual   = xs.contains(Foreground(EitherColor.Black))
        val expected = true
