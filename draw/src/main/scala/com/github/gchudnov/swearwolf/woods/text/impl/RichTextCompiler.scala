package com.github.gchudnov.swearwolf.draw.text.impl
    ...
    def iterate(acc: Array[Byte], state: State, s: RichStyle): Array[Byte] =
      s match
        ...
        case RichForegroundStyle(c, inner) =>
          val prefix    = if !state.fgColor.lastOption.contains(c) then EscSeq.foreground(c).bytes else Array.empty[Byte]
        ...
        case RichBackgroundStyle(c, inner) =>
          val prefix    = if !state.bgColor.lastOption.contains(c) then EscSeq.background(c).bytes else Array.empty[Byte]
        ...
        case RichBoldStyle(inner) =>
          val prefix    = if state.bold.isEmpty then EscSeq.bold.bytes else Array.empty[Byte]
          val updStyles = state.copy(bold = state.bold.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.bold.isEmpty then EscSeq.resetBold.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix

        case RichItalicStyle(inner) =>
          val prefix    = if state.italic.isEmpty then EscSeq.italic.bytes else Array.empty[Byte]
          val updStyles = state.copy(italic = state.italic.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.italic.isEmpty then EscSeq.resetItalic.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix

        case RichUnderlineStyle(inner) =>
          val prefix    = if state.underline.isEmpty then EscSeq.underline.bytes else Array.empty[Byte]
          val updStyles = state.copy(underline = state.underline.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.underline.isEmpty then EscSeq.resetUnderline.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix

        case RichBlinkStyle(inner) =>
          val prefix    = if state.blink.isEmpty then EscSeq.blink.bytes else Array.empty[Byte]
          val updStyles = state.copy(blink = state.blink.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.blink.isEmpty then EscSeq.resetBlink.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix

        case RichInvertStyle(inner) =>
          val prefix    = if state.invert.isEmpty then EscSeq.invert.bytes else Array.empty[Byte]
          val updStyles = state.copy(invert = state.invert.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.invert.isEmpty then EscSeq.resetInvert.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix

        case RichStrikethroughStyle(inner) =>
          val prefix    = if state.strikethrough.isEmpty then EscSeq.strikethrough.bytes else Array.empty[Byte]
          val updStyles = state.copy(strikethrough = state.strikethrough.appended(()))
          val nested    = nestedStyles(updStyles, inner)
          val suffix    = if state.strikethrough.isEmpty then EscSeq.resetStrikethrough.bytes else Array.empty[Byte]
          acc ++ prefix ++ nested ++ suffix
